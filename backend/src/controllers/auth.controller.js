...
export const signup = async (req, res, next) => {
  ...
  const newUser = new User({
    //on n'a pas mis username:username car les noms sont identique alors ce ne plus obligatoire
    username,
    email,
...
export const signin = async (req, res, next) => {
  ...
  try {
    ...
    const token = jwt.sign(
      ...
      { expiresIn: "7d" }
    ); //dans ce token j'ai aussi ajouter isAdmin
    ...
    res
      ...
      .cookie("access_token", token, {
        //je renvoi le token sous forme de cookie
        httpOnly: true,
...
export const google = async (req, res, next) => {
  //cette fonction va permettre de signin ou signup avec votre compte google que vous allez récupérer
    ...
    if (!user) {
      ...
      const newUser = new User({
        username:
          ...
          Math.random().toString(9).slice(-4), //en prenant le nom je l'ai mis en miniscule,supprimer les spaces et mis des nombres aléatoires au devant pour le rendre unique
        email: email,
        password: hashedPassword,
      ...
      const token = jwt.sign(
        { id: aUser._id, isAdmin: aUser.isAdmin },
        process.env.JWT_SECRET_KEY,
        { expiresIn: "7d" }
      );
